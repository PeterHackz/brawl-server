...
public final class TweetNacl {
  ...
  public static final class Box {
    ...
    public byte[] box(byte[] message, byte[] theNonce) {
      ...
      for (int i = 0; i < message.length; i++) m[i + zerobytesLength] = message[i];

      if (0 != crypto_box(c, m, m.length, theNonce, theirPublicKey, mySecretKey)) return null;
    ...
    public byte[] open(byte[] box, byte[] theNonce) {
      ...
      for (int i = 0; i < box.length; i++) c[i + boxzerobytesLength] = box[i];

      if (0 != crypto_box_open(m, c, c.length, theNonce, theirPublicKey, mySecretKey)) return null;
    ...
    public byte[] after(byte[] message, byte[] theNonce) {
      // check message
      if (!(message != null && theNonce != null && theNonce.length == nonceLength)) return null;
  ...
  public static final class Signature {
   ...
   // private static final String TAG = "Signature";
    ...
    public byte[] open(byte[] signedMessage) {
      // check sm length
      if (!(signedMessage != null && signedMessage.length > signatureLength)) return null;
      ...
      for (int i = 0; i < msg.length; i++) msg[i] = signedMessage[i + signatureLength];
  ...
  private static int vn(
      byte[] x, final int xoff, final int xlen, byte[] y, final int yoff, final int ylen, int n) {
    int i, d = 0;
  ...
  private static int crypto_verify_16(
      byte[] x, final int xoff, final int xlen, byte[] y, final int yoff, final int ylen) {
  ...
  private static void core(byte[] out, byte[] in, byte[] k, byte[] c, int h) {
    ...
    } else for (i = 0; i < 16; i++) st32(out, 4 * i, 4, x[i] + y[i]);
  ...
  private static int crypto_stream_salsa20_xor(
      byte[] c, byte[] m, long b, byte[] n, final int noff, final int nlen, byte[] k) {
  ...
  private static int crypto_stream_salsa20(
      byte[] c, long d, byte[] n, final int noff, final int nlen, byte[] k) {
  ...
  private static void sel25519(
      long[] p, final int poff, final int plen, long[] q, final int qoff, final int qlen, int b) {
    int i;
    long t, c = ~(b - 1);
  ...
  private static void S(
      long[] o, final int ooff, final int olen, long[] a, final int aoff, final int alen) {
    M(o, ooff, olen, a, aoff, alen, a, aoff, alen);
  }

  private static void inv25519(
      long[] o, final int ooff, final int olen, long[] i, final int ioff, final int ilen) {
    long[] c = new long[16];
    int a;
  ...
  private static void cswap(long[] p[], long[] q[], byte b) {
    ...
    for (i = 0; i < 4; i++) sel25519(p[i], 0, p[i].length, q[i], 0, q[i].length, b);
  }
  ...
  public static int crypto_sign(
      byte[] sm, long dummy /* *smlen not used*/, byte[] m, int /*long*/ n, byte[] sk) {
  ...
  public static int crypto_sign_open(
      byte[] m, long dummy /* *mlen not used*/, byte[] sm, int /*long*/ n, byte[] pk) {
    int i;
  ...
  public static void randombytes(byte[] x, int len) {
    ...
    if (ret > 0) {
      rnd = jrandom.nextLong();
      for (int i = len - ret; i < len; i++) x[i] = (byte) (rnd >>> 8 * i);
    }
  }
}
